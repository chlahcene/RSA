...
void display_text_Page(int x,int y,int max_char_line,int NumeroPage,int NbrLine,int *index,const char* NomFichier);
    // Affiche un page text dans secreen debutera a la postion x et Numero du page est suivant un index des line dans le fichier
...
void Compte_ouvert(int inveter){
    ...
    int c3=10;
        ...
        for(i=0;i<b;i++) B[i]=malloc(100*sizeof(char));
        strcpy(B[2],"CRYPTOGRAPHY RSA ");
        strcpy(B[0],"Display");
        strcpy(B[3],"Calcule  RSA ");
        strcpy(B[1],"Search and Replace");
        if (inveter) strcpy(B[4],"Retourne au menu principale and save data ");
        else strcpy(B[4],"Ferme Session and save data  ");
        char* C3[] =    {
            ...
            do{
                  ...
                  if (i==3){
                                    ...
                                    if (user_data_Rsa.TPremier==0){
                                            ...
                                            getch();
                                              ...
                                              do{
                                                  ...
                                                  textcolor(BLACK);
                                                  i=Menu(user_data_Rsa.NomPremier,user_data_Rsa.TPremier,PX1,PY1,LNGR1,HTR1,WHITE,LIGHTGRAY);
                                                  if (i==0)break;
                                                  textbackground(WHITE);
                                                  system("CLS");
                                                  textcolor(BLACK);
                                                  ...
                                                  printf("\t\t Apuiyer sur < retoure <-- > pour supprimer cette cle ou d'autre touch pour retourner en arreier ");
                                                  c=getch();
                                                  if (c==8){
                                                    if (AreYouSure(20,10)){
                                                        free((user_data_Rsa.Premier)[i-1]);free((user_data_Rsa.NomPremier)[i-1]);
                                                        (user_data_Rsa.Premier)[i-1]=(user_data_Rsa.Premier)[(user_data_Rsa.TPremier)-1];
                                                        (user_data_Rsa.NomPremier)[i-1]=(user_data_Rsa.NomPremier)[(user_data_Rsa.TPremier)-1];
                                                        (user_data_Rsa.TPremier)--;
                                                        if (user_data_Rsa.TPremier==0) break;
                                                    }
                                                  }
                                    ...
                                    if (user_data_Rsa.TClePublic==0){
                                            ...
                                            getch();
                                              ...
                                              do{
                                                  ...
                                                  system("CLS");
                                                  textcolor(BLACK);
                                                  gotoxy(20,2);printf("Le cle publice Avec le nom %s est :\n\n",(user_data_Rsa.NomClePublic)[i-1]);
                                                  printf("\tn=%s\n\n\te=%s\n\n\t",(user_data_Rsa.ClePublicN)[i-1],(user_data_Rsa.ClePublicE)[i-1]);
                                                  printf("\t\t Apuiyer sur <retoure <-- > pour supprimer cette cle ou d'autre touch pour retourner en arreier ");
                                                  c=getch();
                                                  if (c==8){
                                                    if (AreYouSure(20,10)){
                                                        free((user_data_Rsa.ClePublicN)[i-1]);free((user_data_Rsa.NomClePublic)[i-1]);free((user_data_Rsa.ClePublicE)[i-1]);
                                                        (user_data_Rsa.ClePublicN)[i-1]=(user_data_Rsa.ClePublicN)[(user_data_Rsa.TClePublic)-1];
                                                        (user_data_Rsa.ClePublicE)[i-1]=(user_data_Rsa.ClePublicE)[(user_data_Rsa.TClePublic)-1];
                                                        (user_data_Rsa.NomClePublic)[i-1]=(user_data_Rsa.NomClePublic)[(user_data_Rsa.TClePublic)-1];
                                                        (user_data_Rsa.TClePublic)--;
                                                        if (user_data_Rsa.TClePublic==0) break;
                                                    }
                                                  }
                                              }while(1);
                                        }
                                    ...
                                    if (user_data_Rsa.TCleComplet==0){
                                            ...
                                            getch();
                                                ...
                                                do{
                                                  ...
                                                  if (c==8){
                                                    if (AreYouSure(20,10)){
                                                        free((user_data_Rsa.CleCompletN)[i-1]);free((user_data_Rsa.NomCleComplet)[i-1]);
                                                        free((user_data_Rsa.CleCompletD)[i-1]);free((user_data_Rsa.CleCompletE)[i-1]);
                                                        (user_data_Rsa.CleCompletN)[i-1]=(user_data_Rsa.CleCompletN)[(user_data_Rsa.TCleComplet)-1];
                                                        (user_data_Rsa.CleCompletE)[i-1]=(user_data_Rsa.CleCompletE)[(user_data_Rsa.TCleComplet)-1];
                                                        (user_data_Rsa.CleCompletD)[i-1]=(user_data_Rsa.CleCompletD)[(user_data_Rsa.TCleComplet)-1];
                                                        (user_data_Rsa.NomCleComplet)[i-1]=(user_data_Rsa.NomCleComplet)[(user_data_Rsa.TCleComplet)-1];
                                                        (user_data_Rsa.TCleComplet)--;
                                                        if (user_data_Rsa.TCleComplet==0) break;
                                                    }
                                                  }
                                              }while(1);
                                        }
                   ...
                   /// recharche
                    ...
                    /// Fermer la session
                        if (inveter==0) Save_File_Data_RSA(user_data_Rsa);
                        else Write_RSA_File(user_data_Rsa,"cle rsa.txt");
...
void PrintFile(const char *NomFichier){
 char deliminateur[]=" \t\n,;:)_}>!?&/\\";
    int *index;
    int i=0,j=0,i1=0,j1=0,x=10,y=5,ch,Last_Page,tmp_in,Nbr_mot;
    int LongEcran=30,LargeEcran=130;
    int NumeroPage=1,NbrPage,LinePage=50,line_char_max=80,espace_entre_page=1,videH=5,videL=5;
    index=IndexFileTexte(NomFichier," \t\n,.;:)!?0123456789",line_char_max,&Nbr_mot);
    NbrPage=index[0]/LinePage+1;// le nombre des page
    char* sta1=NULL;
    sta1=malloc(900*sizeof(char));
        textcolor(BLACK);textbackground(WHITE);
        sprintf(sta1,"  Nombre des pages : %d\n  Nombre des mots : %d\n  Nombre des Linges : %d\n  Nombre des Chars : %d",NbrPage,Nbr_mot,index[0],index[index[0]]);
        cputsxyAreaBlanc(30,15,sta1,5,40,"\n : 0123456789");
        getch();
        free(sta1);
    DrawRectangle(x,y,line_char_max+2*videL,LinePage+2*videH);// affiche page plan
    gotoxy(x+videL-3+line_char_max/2,y+(videL*3)/2+LinePage); // affiche nemero de page
    printf("%02d / %02d",NumeroPage,NbrPage);
    display_text_Page(x+videL,y+videH,line_char_max,NumeroPage,LinePage,index,NomFichier);
    i1=1;j1=1;Last_Page=y+LinePage+2*videH;
    gotoxy(i1,j1);
    do{
        ch=SaiserCtrlText();
        if (ch==72){
            ...
            if(NumeroPage<NbrPage && (i1+LongEcran)>=Last_Page){
                    ...
                    textbackground(WHITE);
                    textcolor(BLACK);
                    DrawRectangle(x,y+tmp_in,line_char_max+2*videL,LinePage+2*videH);// affiche page plan
                    gotoxy(x+videL-3+line_char_max/2,y+(videL*3)/2+LinePage+tmp_in); // affiche nemero de page
                    printf("%02d / %02d",NumeroPage,NbrPage);
                    display_text_Page(x+videL,y+videH+tmp_in,line_char_max,NumeroPage,LinePage,index,NomFichier);
            ...
            // touch --
                ...
                if (sta!=NULL){
                    ...
                    sprintf(sta,"  Nombre des pages : %d\n  Nombre des mots : %d\n  Nombre des Linges : %d\n  Nombre des Chars : %d",NbrPage,Nbr_mot,index[0],index[index[0]]);
             ...
             do{
                ...
                if (ch==27) {
                    ...
                    for(jkl=1;jkl<=cord[0];jkl++){
                        ...
                        tmp_in=((hg+1)/LinePage)*(LinePage+2*videH+espace_entre_page);
                        textbackground(YELLOW);
                        if (cord[jkl]<index[hg]) gotoxy(x+videL+cord[jkl],y+videH+tmp_in+(hg)%LinePage-1);
                        else gotoxy(x+videL+cord[jkl]-index[hg],y+videH+tmp_in+(hg)%LinePage-1);
                        printf("%s",mot);
                        do{
                            ch=getch();
                        }while(ch!=13 && ch!=27);
...
void display_text_Page(int x,int y,int max_char_line,int NumeroPage,int NbrLine,int *index,const char* NomFichier){
    ...
    if (j>0) debut=index[j];
    j=j+NbrLine;
    if (j<=index[0]) fin=index[j]-1;else fin=index[index[0]]-1;
...
void recharche_ramplacer(const char* NomFichier){
    ...
    if (Lire_mot_recharche(mot,nouveau_mot)) return 1;
    int *index;
    ...
    if (stacord!=NULL){
        ...
        for(jkl=1;jkl<=cord[0];jkl++){
            ...
            if (hg<=index[0]){
             ...
             sprintf(stacord,"%s\n  %d : Page %d Line %d Position %d",stacord,jkl,(hg+1)/LinePage+1,(hg)%LinePage+1,jj);
        }
            ...
            if (i==1){
                    ...
                    while (fgets(Line1,255,F)!=NULL){
                        ...
                        while((k1=str_istr(Line1+c1,mot))>-1 && Line1[c1]!=0){
                                ...
                                //printf("%s",Line1);
                                i1=0;
                                while(Line1[i1]!=0) putch((unsigned char)(Line1[i1++]));
                                //puts(Line1);
                                textcolor(BLACK);textbackground(YELLOW);
                                gotoxy(c1+k1+1,5);
                                printf("%s",mot);
                    ...
                    fclose(F);fclose(G);
                    remove(NomFichier);
                    if (rename("tmppppp.txt",NomFichier)) fprintf(stderr,"Erreur de Renamation");;
                    free(Line1);
                    }else fprintf(stderr,"Erreur de alocation");
            ...
            printf("Occurance %d : Page %d Line %d Position %d",jkl,(hg+1)/LinePage+1,(hg)%LinePage+1,jj);
            if (AreYouSure(15,7)) ramplace_boulean[jkl-1]=1;
                else ramplace_boulean[jkl-1]=0;

            }*/
            }else {
                for(jkl=1;jkl<=cord[0];jkl++) ramplace_boulean[jkl-1]=1;
            ...
            system("CLS");
            textcolor(WHITE);
            cputsxyTemp(15,5,"Le remplacemant est terminer avec succes merci ... ",15);
        FI:
         free(index);
...
void Ramplace_File(const char* NomFichier,const char* mot,const char* nouveaux_mot,int* ramplace_boulean){
    FILE* F=fopen(NomFichier,"r");
    FILE* G=fopen("tmppppp.txt","w");
    int j=0,k,c;
    char* Line;
    ...
    while (fgets(Line,255,F)!=NULL){
        ...
        while((k=str_istr(Line+c,mot))>-1 && Line[c]!=0){
            if (ramplace_boulean[j++]) Str_Ramplace_mot(Line+c,mot,nouveaux_mot);
            c+=k+1;
        }
        fprintf(G,"%s",Line);
    }
...
gotoxy(z,w+1);
printf("                     %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c     ",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+2);
printf("                     %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c      ",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+3);
printf("                     %c%c       %c%c   %c%c             %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
gotoxy(z,w+4);
printf("                     %c%c       %c%c   %c%c             %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+5);
printf("                     %c%c       %c%c   %c%c             %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+6);
printf("                     %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c    ",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+7);
printf("                     %c%c%c%c%c%c%c%c%c     %c%c%c%c%c%c%c%c%c      %c%c%c%c%c%c%c%c%c     ",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+8);
printf("                     %c%c %c%c                %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a);
gotoxy(z,w+9);
printf("                     %c%c   %c%c              %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+10);
printf("                     %c%c    %c%c             %c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+11);
printf("                     %c%c     %c%c     %c%c%c%c%c%c%c%c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z,w+12);
printf("                     %c%c      %c%c    %c%c%c%c%c%c%c%c%c      %c%c     %c%c",a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a);
 gotoxy(z+20,w+13);
printf("\n");
...
printf("\n");
   ...
   textcolor(5);
printf("\n");
   printf("                                        |%c%c           SECTION A    --|-->    2CPI   --|-->    GO4                      %c%c|",a,a,a,a);
   textcolor(5);
printf("\n");
   textcolor(5);
...
printf("\n");
   ...
   textcolor(5);
printf("\n");
   printf("                                        |%c%c           ENCADRE PAR MR KERMI ADEL                                        %c%c|",a,a,a,a);
   textcolor(5);
printf("\n");
   textcolor(5);
   printf("                                        |%c%c____________________________________________________________________________%c%c|",a,a,a,a);
