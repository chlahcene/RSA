...
void Crypt_Chain_RSA(const char *chain, int taille, BigInt n, BigInt e,
                     BigInt *code_Chain_Crypter);
// crypt chain avec cle (n,e)
void Decryp_Chain_RSA(BigInt code_Chain_A_decrypter, BigInt n, BigInt d,
                      char **chain_decrypter);
// Decrypt code_Chain_A_decrypter avec cle prive (n,d)
void Crypt_Fichier(const char *NomFichierACrypt, const char *NouveauxFichier,
                   BigInt n, BigInt e);
// CRYPTER LE FICHIER NomFichierACrypt AVEC LA CLE (n,e)
void Decrypt_Fichier(const char *NomFichierADecrypt,
                     const char *NouveauxFichier, BigInt n, BigInt d);
...
// crypeter un chaine avec RSA avec cl� public sans avoir le cl� priv�
...
void Genere_Nbr_Premier(BigInt *Nombre, int tailleNombre,
...
void Ajoute_cle_Public(UtilisateurRSA *user, BigInt n, BigInt e,
                       const char *Nom);
void Ajoute_cle_complet(UtilisateurRSA *user, BigInt n, BigInt e, BigInt d,
                        const char *Nom);
...
void Create_new_RSA_Account(void) {
  ...
  int NumbreInput = 1;
  int c = 0;
  char Username[48] = "\0", LastName[48] = "\0", FirstName[48] = "\0",
       Password[24] = "\0", ConfirmPassword[24] = "\0";
  textbackground(WHITE);
  textcolor(BLACK);
  do {
    if (NumbreInput > 0 && NumbreInput < 6) {
      ...
      if (c == 13) {
        ...
        if (!(Username[0] && LastName[0] && FirstName[0] && Password[0] &&
              ConfirmPassword[0])) {
          // This is input vide
        ...
        if (strcmp(Password, ConfirmPassword) != 0) {
          ...
          AfficheErreur(x + 5, y + 5,
                        "The Password and Confirm Password no match");
          textcolor(BLACK);
          textbackground(WHITE);
          continue;
        }
        ...
        if (F == NULL) {
          ...
          cputsxyTemp(
              22, 3,
              "The create the new account end with success thanks you ...", 10);
        }
        c = getch();
        fclose(F);
        return 0;
...
char *hachage(const char *chain) {
  ...
  BigInt n, e, x;
  mpz_init_set_str(
      n, "29961950402435000214455332031185772183148942025143725898858497", 10);
...
void Write_RSA_File(UtilisateurRSA user, const char *Nom_Fichier) {
  ...
  if (F == NULL) {
    ...
    for (i = 0; i < user.TClePublic; i++)
      fprintf(F, "%s %s %s\n", user.ClePublicN[i], user.ClePublicE[i],
              user.NomClePublic[i]);

    for (i = 0; i < user.TCleComplet; i++)
      fprintf(F, "%s %s %s %s\n", user.CleCompletN[i], user.CleCompletE[i],
              user.CleCompletD[i], user.NomCleComplet[i]);
...
void Read_RSA_File(const char *Nom_Fichier, UtilisateurRSA *user) {
  ...
  if (F == NULL) {
    ...
    if (tmp != NULL) {
      fscanf(F, "%d %d %d\n", &(user->TPremier), &(user->TClePublic),
             &(user->TCleComplet));
      user->Premier = malloc((user->TPremier + 2) * sizeof(*(user->Premier)));
      user->NomPremier =
          malloc((user->TPremier + 2) * sizeof(*(user->NomPremier)));
      user->ClePublicN =
          malloc((user->TClePublic + 2) * sizeof(*(user->ClePublicN)));
      user->ClePublicE =
          malloc((user->TClePublic + 2) * sizeof(*(user->ClePublicE)));
      user->NomClePublic =
          malloc((user->TClePublic + 2) * sizeof(*(user->NomClePublic)));
      user->CleCompletN =
          malloc((user->TCleComplet + 2) * sizeof(*(user->CleCompletN)));
      user->CleCompletE =
          malloc((user->TCleComplet + 2) * sizeof(*(user->CleCompletE)));
      user->CleCompletD =
          malloc((user->TCleComplet + 2) * sizeof(*(user->CleCompletD)));
      user->NomCleComplet =
          malloc((user->TCleComplet + 2) * sizeof(*(user->NomCleComplet)));
      int i;
      char *ptr;
      for (i = 0; i < user->TPremier; i++) {
        fscanf(F, "%s ", tmp);
        (user->Premier)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->Premier)[i])));
        ...
        if (ptr != NULL)
          *ptr = '\0';
        (user->NomPremier)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->NomPremier)[i])));
        strcpy((user->NomPremier)[i], tmp);
      }
      for (i = 0; i < user->TClePublic; i++) {
        fscanf(F, "%s ", tmp);
        (user->ClePublicN)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->ClePublicN)[i])));
        strcpy((user->ClePublicN)[i], tmp);
        fscanf(F, "%s ", tmp);
        (user->ClePublicE)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->ClePublicE)[i])));
        ...
        if (ptr != NULL)
          *ptr = '\0';
        (user->NomClePublic)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->NomClePublic)[i])));
        strcpy((user->NomClePublic)[i], tmp);
      }
      for (i = 0; i < user->TCleComplet; i++) {
        fscanf(F, "%s ", tmp);
        (user->CleCompletN)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->CleCompletN)[i])));
        strcpy((user->CleCompletN)[i], tmp);
        fscanf(F, "%s ", tmp);
        (user->CleCompletE)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->CleCompletE)[i])));
        strcpy((user->CleCompletE)[i], tmp);
        fscanf(F, "%s ", tmp);
        (user->CleCompletD)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->CleCompletD)[i])));
        strcpy((user->CleCompletD)[i], tmp);
        fgets(tmp, 1000, F);
        ptr = strchr(tmp, '\n');
        if (ptr != NULL)
          *ptr = '\0';
        (user->NomCleComplet)[i] =
            malloc((strlen(tmp) + 1) * sizeof(*((user->NomCleComplet)[i])));
  ...
  do {
    if (NumbreInput > 0 && NumbreInput < 3) {
      ...
      if (c == 13) {
        ...
        cputsxyTemp(10, 5,
                    "l'ouverteur du votre compte est terminer avec succes nous "
                    "avier deja chrger votre data tu peux les utiliser merci\n",
                    20);
        system("PAUSE");
        Compte_ouvert(0);
        return 0;
...
void liberer_sup_Continu_UtilisateurRSA(UtilisateurRSA *user) {
  ...
  free(user->Premier);
  free(user->NomPremier);
  for (i = 0; i < user->TClePublic; i++) {
    free((user->ClePublicN)[i]);
    free((user->ClePublicE)[i]);
    free((user->NomClePublic)[i]);
  }
  free(user->ClePublicN);
  free(user->ClePublicE);
  free(user->NomClePublic);
  for (i = 0; i < user->TCleComplet; i++) {
  ...
  free(user->CleCompletN);
  free(user->CleCompletE);
  free(user->CleCompletD);
  free(user->NomCleComplet);
  user->TPremier = 0;
  user->TCleComplet = 0;
  user->TClePublic = 0;
}
...
void Ajoute_cle_complet(UtilisateurRSA *user, BigInt n, BigInt e, BigInt d,
  ...
  (user->TCleComplet)++;
  user->CleCompletN =
      realloc(user->CleCompletN,
              (user->TCleComplet + 2) * sizeof(*(user->CleCompletN)));
  user->CleCompletE =
      realloc(user->CleCompletE,
              (user->TCleComplet + 2) * sizeof(*(user->CleCompletE)));
  user->CleCompletD =
      realloc(user->CleCompletD,
              (user->TCleComplet + 2) * sizeof(*(user->CleCompletD)));
  user->NomCleComplet =
      realloc(user->NomCleComplet,
              (user->TCleComplet + 2) * sizeof(*(user->NomCleComplet)));
  char *tmp;
  tmp = malloc(10000000 * sizeof(char));
  ...
  strcpy((user->CleCompletD)[i], tmp);
  (user->NomCleComplet)[i] =
      malloc((strlen(Nom) + 1) * sizeof(*((user->NomCleComplet)[i])));
...
void Ajoute_cle_Public(UtilisateurRSA *user, BigInt n, BigInt e,
  ...
  (user->TClePublic)++;
  user->ClePublicN = realloc(user->ClePublicN, (user->TClePublic + 2) *
                                                   sizeof(*(user->ClePublicN)));
  user->ClePublicE = realloc(user->ClePublicE, (user->TClePublic + 2) *
                                                   sizeof(*(user->ClePublicE)));
  user->NomClePublic =
      realloc(user->NomClePublic,
              (user->TClePublic + 2) * sizeof(*(user->NomClePublic)));
  char *tmp;
  tmp = malloc(10000000 * sizeof(char));
  ...
  (user->NomClePublic)[i] =
      malloc((strlen(Nom) + 1) * sizeof(*((user->NomClePublic)[i])));
...
void Ajoute_Nbr_premier(UtilisateurRSA *user, BigInt nbr, const char *Nom) {
  ...
  (user->TPremier)++;
  user->Premier =
      realloc(user->Premier, (user->TPremier + 2) * sizeof(*(user->Premier)));
  user->NomPremier = realloc(user->NomPremier, (user->TPremier + 2) *
                                                   sizeof(*(user->NomPremier)));
  char *tmp;
  tmp = malloc(10000000 * sizeof(char));
  ...
  (user->NomPremier)[i] =
      malloc((strlen(Nom) + 1) * sizeof(*((user->NomPremier)[i])));
...
void Genere_Cle_Aleatoire(UtilisateurRSA *user, gmp_randstate_t state) {
  ...
  CalculeCleRSA(p, q, &n, &e, &d);
  printf("\t\tc'est voulez vous ajoute cette cle a votre liste des cle Apuiyer "
         "sur <entre> et <echap> sinon\n\n");
  gmp_printf("n=%Zd\n\ne=%Zd\n\nd=%Zd", n, e, d);
  do {
    c = SaiserCntrl();
  } while (c != 13 && c != 27);
  if (c == 13) {
    ...
    textcolor(WHITE);
    cputsxy(x, y + 7,
            "Entrez le nom que voulez vous donnez a cette cle pour plus vite "
            "le connaitre");
    textbackground(WHITE);
...
void Genere_Premier(UtilisateurRSA *user, gmp_randstate_t state) {
  ...
  textcolor(WHITE);
  cputsxy(x, y + 7,
          "Entrez la taille du nombre premier ENTRE 1  ..  999999999 ");
  textbackground(WHITE);
  ...
  printf("\t\tc'est voulez vous ajoute cette nombre a votre liste des cle "
         "Apuiyer sur <entre> et <echap> sinon\n\n");
  gmp_printf("n=%Zd", n);
  do {
    c = SaiserCntrl();
  } while (c != 13 && c != 27);
  if (c == 13) {
    ...
    textcolor(WHITE);
    cputsxy(x, y + 7,
            "Entrez le nom que voulez vous donnez a cette nombre pour plus "
            "vite le connaitre");
    textbackground(WHITE);
    DrawRectangle(x - 1, y + 9, 30, 2);
    textbackground(WHITE);
...
void Next_Prime(UtilisateurRSA *user) {
  ...
  textcolor(WHITE);
  cputsxy(x, y + 7,
          "Entrez un nombre qui ne depasse pas 1000 chiffre et nous vous donne "
          "un nombre premier proche de lui ");
  textbackground(WHITE);
...
void Test_Propa_Prime(UtilisateurRSA *user) {
  ...
  textcolor(WHITE);
  cputsxy(x, y + 7,
          "Entrez un nombre pour tester sa primetiviter qui ne depasse pas "
          "1000 chiffre ");
  textbackground(WHITE);
  DrawRectangle(x - 1, y + 9, 102, 11);
  textbackground(WHITE);
  ...
  if (i == 0) {
    ...
    if (i == 2) {
      ...
      cputsxyTemp(x, y + 1,
                  "Pour le test vraiment avec la methode des divisions ( Mais "
                  "elle Prends Plus de Temps )",
                  15);
      cputsxyTemp(
          x, y + 2,
          "Apuiyer sur < Entre > pour debuter le test et < echap > sinon \n\n",
          15);
      do {
        c = SaiserCntrl();
      } while (c != 13 && c != 27);
...
void Test_Prime_Div(BigInt nomber, UtilisateurRSA *user) {
  ...
  textcolor(WHITE);
  cputsxy(x, y + 7,
          "Le operation peux prendre plus temps Patianter un Peu ... ");
  ...
  mpz_init(procentage);
  mpz_sqrt(q, n);
  mpz_divexact_ui(procentage, q, 100);
  ...
  if (premier == 0) {
    cputsxyTemp(1, y + 22,
                "Le Nombre n'est pas Premier car il Divisable par : ", 15);
    gmp_printf("%Zd\n\n\t\t", d);
    system("PAUSE");
  } else {
...
void Test_Prime(UtilisateurRSA *user) {
  ...
  textcolor(WHITE);
  cputsxy(x, y + 7,
          "Entrez un nombre pour tester sa primetiviter Avec divisions qui ne "
          "depasse pas 1000 chiffre ");
  textbackground(WHITE);
  DrawRectangle(x - 1, y + 9, 102, 11);
  textbackground(WHITE);
  textcolor(BLACK);
...
void Creer_Cle(UtilisateurRSA *user) {
  ...
  textcolor(WHITE);
  cputsxyTemp(10, 20,
              "Choiser 2 nombre premier du liste des nombre premier qui suive "
              "pour creer cle rsa \n\n",
              15);
  int j = 0, i, c, choix[2] = {0, 0};
  if (user->TPremier == 0) {
    printf("\t\tLA LISTE EST VIDE  AJOUTES DES CLE DANS LA PARTIE CALCULE RSA "
           "MERCI ...");
    getch();
    return 1;
  } else {
    do {
      ...
      textcolor(BLACK);
      i = Menu(user->NomPremier, user->TPremier, PX, PY, LNGR, 40, WHITE,
               LIGHTGRAY);
      if (i == 0)
        return 1;
      textbackground(WHITE);
      ...
      printf("voulez vous choiser cette nombre pour creer cle RSA si oui "
             "Apuiyer sur < entres > sinon <echap>");
      do {
        c = SaiserCntrl();
      } while (c != 13 && c != 27);
      if (c == 13) {
    ...
    mpz_init(e);
    CalculeCleRSA(p, q, &n, &e, &d);
    printf("\t\tc'est voulez vous ajoute cette cle a votre liste des cle "
           "Apuiyer sur <entre> et <echap> sinon\n\n");
    gmp_printf("n=%Zd\n\ne=%Zd\n\nd=%Zd", n, e, d);
    do {
      c = SaiserCntrl();
    ...
    if (NumbreInput > 0 && NumbreInput < 4) {
      ...
      if (c == 13) {
        ...
        if (c == 13) {
          ...
          mpz_init(d);
          if (nombreD[0])
            mpz_set_str(d, nombreD, 10);
          mpz_init_set_str(n, nombreN, 10);
          mpz_init_set_str(e, nombreE, 10);
          textbackground(BLACK);
          system("CLS");
          textcolor(WHITE);
          cputsxy(x, y + 7,
                  "Entrez le nom que voulez vous donnez a cette cle pour plus "
                  "vite le connaitre");
          textbackground(WHITE);
          DrawRectangle(x - 1, y + 9, 30, 2);
          textbackground(WHITE);
          ...
          do {
            ...
            if (c == 13) {
              ...
              textcolor(WHITE);
              cputsxyTemp(x, y + 15,
                          "Le ajoutation est terminera avec succes \n", 15);
              c = getch();
              break;
            } else if (c == 27) {
              if (AreYouSure(x, y))
...
void choix_crypt(UtilisateurRSA user_data_Rsa) {
  ...
  do {
    ...
    textcolor(BLACK);
    i = Menu(user_data_Rsa.NomClePublic, user_data_Rsa.TClePublic, PX1, PY1,
             LNGR1, HTR1, WHITE, LIGHTGRAY);
    if (i == 0)
      if (AreYouSure(30, 35))
        return 1;
      else
    ...
    textcolor(BLACK);
    gotoxy(20, 2);
    printf("Le cle Public Avec le nom %s est :\n\n",
           (user_data_Rsa.NomClePublic)[i - 1]);
    printf("\tn=%s\n\n\te=%s\n\n", (user_data_Rsa.ClePublicN)[i - 1],
           (user_data_Rsa.ClePublicE)[i - 1]);
    printf("\t\t Apuiyer sur < Entre > pour Crypter avec cette Cle ou d'autre "
           "touch pour Retourner en Arreier ");
    c = getch();
    if (c == 13)
      break;
  } while (1);
...
void choix_Decrypt(UtilisateurRSA user_data_Rsa) {
  ...
  do {
    ...
    FrameRectangle(PX1 - 1, PY1 - 1, LNGR1 + 2, HTR1 + 2, 4);
    textcolor(BLACK);
    i = Menu(user_data_Rsa.NomCleComplet, user_data_Rsa.TCleComplet, PX1, PY1,
             LNGR1, HTR1, WHITE, LIGHTGRAY);
    if (i == 0)
      if (AreYouSure(30, 35))
        return 1;
    ...
    printf(
        ...
        (user_data_Rsa.CleCompletE)[i - 1], (user_data_Rsa.CleCompletD)[i - 1]);
    printf("\t\t Apuiyer sur < Entre > pour DECrypter avec cette Cle ou "
           "d'autre touch pour Retourner en Arreier ");
    c = getch();
    if (c == 13)
      break;
  } while (1);
